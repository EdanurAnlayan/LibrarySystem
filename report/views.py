...
def get_report_models(all_times=False):
    ...
    for kutuphane in kutups:
        ...
        if all_times:
            ...
            emanetler = loan_models.Emanet.objects.filter(given_date__gte=first_day_of_month,given_source__library=kutuphane.id)
        ...
        kutup_list.append({'kutuphane': kutuphane, 'tum_kitaplar': tum_kitaplar, 'emanetler': len(emanetler), 'max_category': category, 'price': price_sum["price__sum"]})
